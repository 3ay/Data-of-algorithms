...
void Tree::sort(BNode * node)
{
	...
	for (int i = 0; i < (2 * t - 1); i++)
	{
		for (int j = i + 1; j <= (2 * t - 1); j++)
		{
			...
			{
				if ((node->keys[i]) < (node->keys[j])) // сравниваем
				{
					m = node->keys[i];
					node->keys[i] = node->keys[j]; // меняем местами
					node->keys[j] = m;
				}
				else break;
			}
...
void Tree::remove_k(int key)
{
	...
	if (searchKey(key, ptr) == false)
	{
		...
		// поиск узла, в которм находится ключ для удаления
		for (i = 0; i < ptr->Count - 1; i++)
		{
			if (ptr->keys[i] != 0)
			{
				if (key == ptr->keys[i]) // непосредственно поиск, совпадение или нет
				{
					position = i;
					break;
				}
				else
				{
					if ((key < ptr->keys[i]))
					{
						ptr = ptr->children[i]; // идём дальше
						positionSon = i + 1;
						i = -1;
					}
				}
	...
	if (ptr->Leaf == true) // достигли конца древа
	{
		if (ptr->Count > (t - 1)) removeFromNode(key, ptr); // удалили ключ из узла
...
void Tree::deletenode(BNode *node)
{
	if (node != nullptr)
	{
		for (int i = 0; i <= (2 * t - 1); i++)
		{
			if (node->children[i] != nullptr) deletenode(node->children[i]); // рекурсивный вызов, пока не дойдём до несущ. дитя (тип лиф)
			else
			{
				delete (node);
				break;
